...
func GetJson(url string, user string, password string, target interface{}) error {
  ...
  fmt.Printf("Get JSON from URL: %s\n", url)
...
func GetLogs(jenkins string, user string, password string, build string, pipeline string) error {
  fmt.Printf("GetLogs started: %s %s %s\n",jenkins, build, pipeline)
	runs := RunsApi{}
  ...
  fmt.Printf("Found %d Runs\n",len(runs))
	if build == "0" {
      ...
      fmt.Printf("ID=%s State=%s Result=%s, URL=%s%s\n", run.ID, run.State, run.Result, jenkins, run.Links.Nodes.Href)
